...
#'\code{normSens}     \tab numeric \tab Bayesian linear regression for NARFCS\cr
...
#'\code{logregSens}   \tab binary  \tab Logistic regression for NARFCS \cr
...
#'\code{2l.lmm}       \tab numeric  \tab Linear mixed model, approximate bayesian, lmer\cr
...
#'imputations.
#'@param predictorSens For the NARFCS procedure only - use in conjunction with methods "normSens" 
#'or "logregSens". A square matrix similar to \code{predictorSens} (of size \code{ncol(data)}, 
#'with 0/1 data) but specifying the set of predictors to be used in the unidentified 
#'part of the imputation model, i.e. a value of '1' means that the column
#'variable is used as a predictor for that part of the imputation model for the row 
#'variable. The default for \code{predictorSens} is that no variables are included 
#'in this part of the model, so only an intercept term is included. 
#'Note: codes '2' and '-2' are not allowed here. 
#'@param formSens For the NARFCS procedure only - use in conjunction with methods "normSens" 
#'or "logregSens". Similar to \code{form} it is an alternative to the matrix option 
#'(\code{predictorSens}) to specify the predictors in unidentified 
#'part of the imputation model. It is a vector of strings of length \code{ncol(data)}, 
#'specifying formulae. The jth string is for the jth variable in the data, and is parsed and 
#'executed within the \code{sampler()} function to create the design matrix for the 
#'unidentified part of the imputation model for that variable. An empty string results 
#'in this argument being ignored and the corresponding row of the \code{predictorSens} 
#'argument being used instead to the determine which predictors enter this part of the model. 
#'The default is a vector of empty strings \code{''} so this argument is entirely ignored. 
#'As for \code{form}, the main value lies in the easy specification of interaction terms. 
#'@param parmSens For the NARFCS procedure only - use in conjunction with methods "normSens" 
#'or "logregSens". This argument enables the user to provide values for the sensitivity 
#'parameters in the unidentified part of the imputation model, which must be specified in the log-odds 
#'scale when imputing a binary variable ("logregSens" method). \code{parmSens} is a list object with 
#'one element per variable in the order in which they appear in the dataset, and its
#'elements are lists of vectors. For variables in the dataset that are complete or to be
#'imputed via standard FCS (non-Sens) methods, the corresponding element is a list with an empty string \code{''} 
#'as unique element (this is the default for all variables). For variables to be imputed via NARFCS, it is
#'a list of length equal to 1 (for the intercept) plus the number of variables included as predictors 
#'in the unidentifiable part of the imputation model, as speficied through \code{predictorSens} or 
#'\code{formSens}. The first component is the sensitivity parameter corresponding to the intercept (a scalar) 
#'and the following components are sensitivity parameter vectors (of length 1 except for factors - see below)
#'for the predictor variables provided in the order in which they appear in the dataset 
#'(if using the matrix syntax) or in which they were provided in the formula (if using the formula syntax). 
#'For a factor predictor variable with K categories, the element is a vector of 
#'length K-1, containing the sensitivity parameters for the K-1 indicator variables of the 
#'non-reference categories, following the order of the levels of that factor variable. 
#'These parameters are interpreted relative to the reference category.
#'
...
mice <- function(data, m = 5, 
                 ...
                 data.init = NULL, 
                 predictorSens=(diag(0, ncol(data))),
                 formSens= vector("character", length = ncol(data)), 
                 parmSens= rep(list(list("")), ncol(data)), ...) {
  ...
  midsobj <- list(call = call, data = as.data.frame(p$data[, seq_len(nvar)]), 
                  ...
                  predictorMatrix = predictorMatrix,
                  visitSequence = visitSequence, form = form, 
                  predictorSens=predictorSens,
                  formSens=formSens,
                  parmSens=parmSens,
                  post = post,
