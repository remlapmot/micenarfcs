  ...
  # Authors: S van Buuren, K Groothuis-Oudshoorn
{
  ...
  if (maxit >= 1) {
    ...
    for (k in from:to) {
      ...
      for (i in seq_len(m)) {
        ...
        for (j in visitSequence) {
          ...
          p$dataSens[(!ry) & wy, j] <- imp[[j]][(!ry)[wy], i]
        }
        ...
        for (j in setdiff(p$visitSequence, visitSequence)) {
          ...
          cat.columns <- p$dataSens[, p$categoriesSens[j, 4]]
          mmSens<-model.matrix(~ cat.columns - 1, model.frame(~ cat.columns, na.action = na.pass))[, -1]
          p$dataSens[, (j:(j + p$categoriesSens[p$categoriesSens[j,4], 2] - 1))] <-mmSens
          }
        ...
        for (j in p$visitSequence) {
          ...
          if (elem) {
            ...
            if (! is.null(p$formSens) &&nchar(p$formSens[j])>0) {
              myformSens <- paste(p$formSens[j], "0", sep = "+")
              xSens <- model.matrix(formula(myformSens), p$dataSens)
            }else 
              xSens <- p$dataSens[, p$predictorSens[j,] == 1, drop = FALSE]
            ...
            x <- x[, keep, drop = FALSE]
            type <- type[keep]
            keep <- remove.lindep(xSens, y, ry, ...)
            xSens <- xSens[, keep, drop = FALSE]
            parmS<-unlist(p$parmSens[[j]])
            
            if(theMethod=="normSens"|theMethod=="logregSens")
            {
             if(k==from&i==1)
            {
              VarSens[[j]]<-c("(Intercept)",dimnames(xSens)[[2]])
              SPbyVar[[j]]<-parmS
              names(VarSens)[j]<-names(SPbyVar)[j]<-names(p$data)[j]
            }}
            ...
            imputes[cc] <- do.call(f, args = list(y, ry, x, wy = wy, type = type,
                                                  xSens=xSens, parmS=parmS,...))
            imp[[j]][, i] <- imputes
            p$data[(!r[, j]) & where[, j], j] <- imp[[j]][(!r[, j])[where[, j]], i]
            p$dataSens[(!r[, j]) & where[, j], j] <- imp[[j]][(!r[, j])[where[, j]], i]
          ...
          if (dumm) {
            ...
            cat.columns <- p$dataSens[, p$categoriesSens[j, 4]]
            mmSens <- model.matrix(~ cat.columns - 1, 
                               model.frame(~ cat.columns, na.action = na.pass))[, -1]
    ...
    {  cat("\n")
      for (j in visitSequence)
      { 
        if(p$method[j]=="normSens"|p$method[j]=="logregSens")
        {cat(paste("\n Sensitivity parameters in NARFCS model for ", names(VarSens)[j],":",sep=""))
          cat("\n")
          print(cbind(Predictor=VarSens[[j]],SP=SPbyVar[[j]]))
        }
        cat("\n")
      }
      cat("\n")
